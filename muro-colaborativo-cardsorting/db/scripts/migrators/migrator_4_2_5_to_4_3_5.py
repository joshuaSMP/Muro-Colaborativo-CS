...
* La migracion no require manipular los datos; si contamos con los triggers
  los datos se completarian automaticamente omitiendo id
"""
import psycopg2
from abstract_migrator import Migrator

class Migrator_4_2_5_to_4_3_5(Migrator):

    def __init__(self, user, pswd, host, port, originDbName, destDbName):
        super(Migrator_4_2_5_to_4_3_5, self).__init__(user, pswd, host, port,
    ...
    def _doMigration(self, verbose):
        ...
        query = "INSERT INTO public.shared_object(id, room, percentage_x, "\
            +"percentage_y, type, cursor, text, creation_date, owners, public_id,"\
            +"image_path, last_edited_date, edited_times, activity_id, "\
            +"is_deleted)"\
